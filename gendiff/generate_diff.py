...
def compile_info(key, value, status):
    if status != NO_CHANGE_STATUS and isinstance(value, dict):
        return (key, {
            'status': status,
            'value': dict(map(
                lambda k: compile_info(k, value[k], 'nothing changed'),
                value.keys()))})
    elif status == CHANGED_STATUS:
        value = [dict(map(
            lambda k: compile_info(k, v[k], INNER_CHANGE),
            v.keys()))
            if isinstance(v, dict)
            else v
            for v in value]
        value = [{'status': DELETED_STATUS,
                  'value': value[0]},
                 {'status': ADDED_STATUS,
                  'value': value[1]}]
...
def make_diff(before_dict, after_dict):
    ...
    added_items = dict(map(
        lambda key: compile_info(key, after_dict[key], ADDED_STATUS),
        added_keys))
    deleted_keys = sorted(list(before_dict.keys() - after_dict.keys()))
    deleted_items = dict(map(
        lambda key: compile_info(key, before_dict[key], DELETED_STATUS),
        deleted_keys))
    ...
    for key in intersec:
        if isinstance(before_dict[key], dict) \
          and isinstance(after_dict[key], dict):
            ...
            adjusted_intersec.update(dict([compile_info(key,
                                                        adjusted_value,
                                                        NO_CHANGE_STATUS)]))
